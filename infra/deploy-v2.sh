...
CHANNEL_CFG="$(build_cfg default.agent_id "${DEFAULT_AGENT_ID:-auto}")"
...
WORKLOADS=(
    ...
    "mycelium-channel-router;channel-router;channel-router,agent-registry;wasi:keyvalue:store|wasi:config:store${CHANNEL_CFG:+:}${CHANNEL_CFG}|wasi:logging:logging|wasmcloud:messaging:consumer,handler,types:subscriptions=mycelium.channel.telegram.raw,mycelium.step.result;${POOL_SIZE_DEFAULT}"
